    ...
    decidePath() {
        ...
        if (type === 'normal') {
            ...
            this.http.get(`${this.requestUrl}/thank_you/${file}`).subscribe(thankYouData => {
                this.route.navigate(['complete', Object.assign({ userId: userID }, thankYouData)]);
            });
        }
    }
    ngOnInit() {
...
let GlobalService = class GlobalService {
    ...
    checkRoute() {
        ...
        if (type === 'normal') {
            ...
            const file = pathArray[pathIndex]['file'];
            this.http.get(`${this.requestUrl}/thank_you/${file}`).subscribe(thankYouData => {
                this.router.navigate(['complete', Object.assign({ userId: userID }, thankYouData)]);
            });
        }
        else if (type === 'qv') {
            return true;
    ...
    submit(finalQuestionValue) {
        ...
        if (nextQuestionIndex >= this.questionnaire.question_list.length) {
            ...
            if (pathArray[pathIndex + 1]['type'] === 'donation') {
                ...
                return this.http.post(`${this.requestUrl}/submit`, { submitData, finalQuestion: finalQuestionValue }).pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_3__["catchError"])(this.handleError)).subscribe(data => {
