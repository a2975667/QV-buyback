...
(window["webpackJsonp"] = window["webpackJsonp"] || []).push([["main"], {
  ...
  function srcAppVideoVideoComponentTs(module, __webpack_exports__, __webpack_require__) {
    ...
    /*#__PURE__*/
    function () {
      function VideoComponent(http, vService, cookieService, route) {
      ...
      _createClass(VideoComponent, [{
        ...
        key: "onCheckboxClick",
        value: function onCheckboxClick(event, key) {
        ...
        value: function ngOnInit() {
          ...
          this.vService.videoForm.subscribe(function (data) {
            if (data) {
              ...
              if (_this24.saveApply.apply) {
                _this24.videoConfig = JSON.parse(_this24.cookieService.get('video_config'));
                _this24.configurations = {
                  'Audio Quality': _this24.videoConfig[0],
                  'Video Resolution': _this24.videoConfig[1],
                  'Audio Stability': _this24.videoConfig[2],
                  'Motion Smoothness': _this24.videoConfig[3],
                  'Audio-Video Synchronization': _this24.videoConfig[4]
                };
              }

              var userID = _this24.cookieService.get('user_id');

              _this24.description = data.Description;
              _this24.title = data.Title;
              _this24.showCost = data.settings.control_panel_has_price;
              _this24.showConfig = data.settings.control_panel_can_change;
              _this24.showDemoConfig = data.settings.showDemoConfig;
              _this24.showFullConfig = data.settings.showFullConfig;
              var time = Date.now().toString();
              _this24.videoSrc = _this24.videoFilePrefix + _this24.formJson.filename + '-vq' + _this24.configurations['Video Resolution'] + '.webm?t=' + time + '&userId=' + userID;
              _this24.audioSrc = _this24.audioFilePrefix + _this24.formJson.filename + '-av' + _this24.configurations['Audio Quality'] + '.m4a?t=' + time + '&userId=' + userID;
              _this24.videoElement.src = _this24.videoSrc;
              _this24.audioElement.src = _this24.audioSrc;
              _this24.showPanel = data.settings.control_panel_can_change;

              _this24.refreshPlayback();
            }
          });
        }
      }, {
        ...
        value: function decidePath() {
          ...
          if (type === 'normal') {
            ...
            this.http.post("".concat(this.requestUrl, "/submit"), {
              ...
              _this25.http.get("".concat(_this25.requestUrl, "/thank_you/").concat(file)).subscribe(function (thankYouData) {
                _this25.route.navigate(['complete', Object.assign({
                  userId: userId
                }, thankYouData)]);
              });
            });
          ...
          if (this.conditionOne && this.conditionTwo && this.conditionThree) {
            if (!this.cookieService.check('user_id')) {
              ...
              this.gService.getUserID(userGP).subscribe(function (user) {
                _this27.initCookie(user);

                if (user.path_id === 'thank_you') {
                  var userID = _this27.cookieService.get('user_id');

                  _this27.cookieService.deleteAll('/');

                  _this27.router.navigate(['complete', {
                    userId: userID,
                    text: null,
                    title: null
                  }]);
                } else {
                  _this27.router.navigate(['demographic']);
                }
              });
            } else {
